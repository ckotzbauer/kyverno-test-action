...
export const fetchPolicies = async function (): Promise<ClusterPolicy[]> {
  ...
  const globber = await glob.create(core.getInput("policy-files", { required: true }), { followSymbolicLinks: false });
...
export const parsePolicyFile = function (content: string): ClusterPolicy[] {
  ...
  for (const policy of policies) {
    ...
    for (const rule of policy.spec.rules) {
      const relatesToPods = rule?.match?.resources?.kinds?.includes("Pod") || rule?.exclude?.resources?.kinds?.includes("Pod");
      const hasPatternOrAntiPattern = rule?.validate?.pattern || rule?.validate?.anyPattern;
